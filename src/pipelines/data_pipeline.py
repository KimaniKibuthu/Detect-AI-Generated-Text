...
def data_pipeline(fit_tokenizer: bool = False, fit_vectorizer: bool = True) -> Tuple:
    ...
    try:
        ...
        tokenized_train_data = tokenize_data(
            processed_train_data, config["models"]["sentpiece_model_path"]
        )
        tokenized_test_data = tokenize_data(
            processed_test_data, config["models"]["sentpiece_model_path"]
        )
        logger.info("Data tokenized successfully")

        # Split data
        logger.info("Splitting data...")
        validation, test = split_data(
            tokenized_test_data, test_size=config["variables"]["test_size"]
        )
        ...
        vectorized_X_train_data = vectorize_data(
            vectorizer, tokenized_train_data["text_spm"]
        )
        vectorized_X_test_data = vectorize_data(vectorizer, test["text_spm"])
        vectorized_X_validation_data = vectorize_data(
            vectorizer, validation["text_spm"]
        )
        ...
        save_data(
            vectorized_X_train_data, config["data"]["modelling_X_train_data_path"]
        )
        save_data(vectorized_X_test_data, config["data"]["modelling_X_test_data_path"])
        save_data(
            vectorized_X_validation_data,
            config["data"]["modelling_X_validation_data_path"],
        )
        ...
        return (
            vectorized_X_train_data,
            vectorized_X_test_data,
            vectorized_X_validation_data,
            y_train_data,
            y_test_data,
            y_validation_data,
...
if __name__ == "__main__":
    data_pipeline(
        config["data_pipeline"]["fit_tokenizer"],
